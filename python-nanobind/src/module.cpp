...
NB_MODULE(_qlnb, m) {
    m.doc() = "Nanobind bindings for QuantLib (phase 123)";
    m.attr("__version__") = "4.55.0";
    ...
    bind_time(m);
    bind_curves(m);
    bind_heston(m);  // before instruments/pricing (set_heston_pricing_engine)
    bind_instruments(m);
    bind_pricing(m);
