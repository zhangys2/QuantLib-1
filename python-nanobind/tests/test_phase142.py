"""Phase-142 tests: FdBlackScholesAsianEngine."""
...
def _fd_option(today, option_type, strike, first, length, fixings, process):
    ...
    # Suite: FdBlackScholesAsianEngine(process, 100, 100, 100).
...
# checked with FD engine when fixings < 100 (tol 2e-2).
_LEVY = [
    (2, 1.3942835683),
    (4, 1.5852442983),
    (8, 1.66970673),
...
def test_fd_asian_levy(fixings, npv):
    ...
    assert opt.NPV() == pytest.approx(npv, abs=2e-2)
    assert opt.is_expired() is False


def test_compat_phase142_aliases():
    import qlnb.compat as cql

    assert hasattr(cql.DiscreteAveragingAsianOption, "setFdPricingEngine")
    assert cql.FdBlackScholesAsianEngine is not None
